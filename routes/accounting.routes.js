 ...
 *
 * USAGE in superadmin_routes.js — replace or extend the three inline
 * accounting endpoints with this dedicated router:
 *
 *   import accountingRouter from './accounting.routes.js';
 *   router.use('/accounting', accountingRouter);
 *
 * Endpoints:
 *   GET    /accounting/transactions          – list with filters, summary
 *   POST   /accounting/transactions/:id/disburse  – mark disbursed + audit
 *   GET    /accounting/transactions/:id/receipt   – PDF receipt (download/preview)
 *   POST   /accounting/transactions/:id/email     – email PDF receipt
 *   GET    /accounting/summary               – totals only (dashboard cards)
 *   GET    /accounting/export                – CSV export of all transactions
 */
...
import {
  hashToken, AUDIT_ACTIONS,
} from '../utils/securityUtils.js';

// ─── Re-export router so it can be mounted in superadmin_routes.js ───────────
const router = express.Router();

// ─── Constants ───────────────────────────────────────────────────────────────

/**
 * Platform profit percentage per package sale.
 * Can also be stored per-package in DB; this is the fallback default.
 */
...
/**
 * Authenticate superadmin via JWT + active session check.
 * Identical to the inline middleware in superadmin_routes.js – extracted
 * here so this file is self-contained and testable independently.
 */
const authenticateSuperadmin = async (req, res, next) => {
  try {
    ...
    // Async session activity update – fire-and-forget
...
// Returns aggregated totals for dashboard cards (fast, no pagination).
// ─────────────────────────────────────────────────────────────────────────────
router.get('/summary', authenticateSuperadmin, async (req, res) => {
  try {
    // Query successful payments joined with packages for percentage
    const { data: payments, error } = await supabase
      ...
      .select(`
        ...
        paid_at,
        disbursed,
        package:packages(price, profit_percentage)
      `)
      .eq('status', 'SUCCESS')
...
// Full transaction list with joins, optional filters.
//
// Query params:
//   limit    – max rows (default 200, max 1000)
//   offset   – pagination offset
//   status   – "pending" | "disbursed" | "all" (default "all")
//   from     – ISO date filter paid_at >= from
//   to       – ISO date filter paid_at <= to
//   q        – free-text search (package name, client/agent email)
// ─────────────────────────────────────────────────────────────────────────────
...
// Mark a successful payment as disbursed to the agent.
// Idempotency: returns 409 if already disbursed.
// ─────────────────────────────────────────────────────────────────────────────
router.post('/transactions/:id/disburse', authenticateSuperadmin, async (req, res) => {
  ...
  try {
    // Fetch payment + package in one shot
    ...
    if (payment.status !== 'SUCCESS') {
      return res.status(422).json({ success: false, message: 'Payment is not successful – cannot disburse' });
    }
    if (payment.disbursed) {
      return res.status(409).json({ success: false, message: 'Funds already disbursed for this transaction' });
    }
    ...
    const { error: updateErr } = await supabase
      ...
      .eq('id', id)
      .eq('disbursed', false); // Optimistic lock – prevent double disburse
    ...
    res.json({
      success:    true,
      message:    'Transaction marked as disbursed',
      data: {
        id,
        disbursedAt:  new Date().toISOString(),
        disbursedBy:  req.superadmin.id,
        agentShare,
      },
    });
  } catch (err) {
...
// Generates and streams a professional PDF receipt.
// Query param:  ?inline=1  → opens in browser tab instead of downloading.
// ─────────────────────────────────────────────────────────────────────────────
router.get('/transactions/:id/receipt', authenticateSuperadmin, async (req, res) => {
  ...
  try {
    ...
    doc.fill('#FFFFFF').fontSize(22).font('Helvetica-Bold')
      .text('UMRAH MARKET', 60, 30);
    doc.fontSize(9).font('Helvetica').fill('#94A3B8')
      .text('Official Agent Disbursement Receipt', 60, 58);
    ...
    const bannerFg    = payment.disbursed ? '#166534' : '#92400E';

    doc.rect(60, 115, doc.page.width - 120, 32).fill(bannerColor);
    doc.fontSize(9).font('Helvetica-Bold').fill(bannerFg)
    ...
    const sectionLabel = (text, x, yPos) => {
      doc.fontSize(7).font('Helvetica-Bold').fill('#64748B')
        .text(text.toUpperCase(), x, yPos, { characterSpacing: 0.8 });
      doc.moveTo(x, yPos + 12).lineTo(x + 180, yPos + 12).stroke('#E2E8F0');
    };
    ...
    sectionLabel('Client Details', col1, y);
    let y1 = y + 18;
    ...
    sectionLabel('Agent Details', col2, y);
    let y2 = y + 18;
    ...
    doc.fontSize(8).font('Helvetica-Bold').fill('#475569')
      .text('FINANCIAL BREAKDOWN', col1 + 8, tableTop + 7);
    ...
    rows.forEach((row, idx) => {
      if (idx === rows.length - 1) {
        doc.rect(col1, ry - 2, doc.page.width - 120, 26).fill('#EFF6FF');
      }
      doc.fontSize(9)
        .font(row.bold ? 'Helvetica-Bold' : 'Helvetica')
        .fill(row.bold ? '#1D4ED8' : '#334155')
    ...
    // Async: mark receipt generated
    supabase.from('payments').update({ receipt_generated: true }).eq('id', id)
      .then(() => {})
      .catch(() => {});
...
// Generates PDF in memory and emails it via SMTP.
// Body: { email?: string }  (falls back to agent email on record)
// ─────────────────────────────────────────────────────────────────────────────
router.post('/transactions/:id/email', authenticateSuperadmin, async (req, res) => {
  ...
  try {
    ...
    const pdfBuffer = await new Promise((resolve, reject) => {
      ...
      doc.fontSize(9).fill('#64748B').text(
        payment.disbursed
          ? '✓ Funds have been disbursed to this agent.'
          : '⏳ Disbursement is still pending.',
        60, 320,
      );
      doc.end();
    });
...
// CSV export of all successful transactions.
// ─────────────────────────────────────────────────────────────────────────────
router.get('/export', authenticateSuperadmin, async (req, res) => {
  try {
    ...
    const columns = [
      'transaction_id', 'package_name', 'client_name', 'client_email',
      'agent_name', 'agent_email', 'agent_number',
      'revenue_kes', 'profit_pct', 'profit_kes', 'agent_share_kes',
      'mpesa_ref', 'paid_at', 'disbursed', 'disbursed_at',
    ];
