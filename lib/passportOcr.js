...
// @vladmandic/face-api) — the call site and return shape below don't need to
// change.
//
// Returns { ok, buffer, mime, width, height } or { ok: false, reason }.
...
export async function extractFacePhoto(buffer, region = FACE_CROP_REGION) {
  try {
    ...
    return {
      ...
      method: 'geometry', // surfaced so logs/analytics can tell this apart from a future ML method
    };
  } catch (err) {
...
export default { extractPassport, compareWithInput, parseMrzDate, extractFacePhoto, terminateOcrWorker };